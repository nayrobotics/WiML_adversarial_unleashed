...
from xRL.utils.torch_util import SquashedNormal


def create_simple_mlp(in_dim, out_dim, hidden_layers, act):
...
class MLPNet(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_layers=None, act=None):
        super().__init__()
...
class DiagGaussianMLPPolicy(MLPNet):
    def __init__(self, state_dim, act_dim, hidden_layers=None, act=None,
                 init_log_std=0.):
...
class DoubleQ(nn.Module):
    def __init__(self, state_dim, act_dim, hidden_layers=None, act=None):
        ...
        self.net_q1 = MLPNet(in_dim=state_dim + act_dim, out_dim=1, hidden_layers=hidden_layers, act=act)
        self.net_q2 = MLPNet(in_dim=state_dim + act_dim, out_dim=1, hidden_layers=hidden_layers, act=act)
...
class DistributionalDoubleQ(nn.Module):
    def __init__(self, state_dim, act_dim, v_min=-10, v_max=10, num_atoms=51, device="cuda", hidden_layers=None, act=None):
        super().__init__()
        if isinstance(state_dim, Sequence):
            state_dim = state_dim[0]
        self.device = device
        self.net_q1 = MLPNet(in_dim=state_dim + act_dim, out_dim=num_atoms, hidden_layers=hidden_layers, act=act)
        self.net_q2 = MLPNet(in_dim=state_dim + act_dim, out_dim=num_atoms, hidden_layers=hidden_layers, act=act)
...
class MLPCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_layers=None, act=None):
        super().__init__()
        if isinstance(state_dim, Sequence):
            state_dim = state_dim[0]
        self.critic = MLPNet(in_dim=state_dim, out_dim=1, hidden_layers=hidden_layers, act=act)
...
def get_activation(act_name):
    if act_name == "elu":
        ...
        return nn.LeakyReLU()
    elif act_name == "gelu": #like ReLu but smoother, not sharper from 0 to high, but curves through negative
        return nn.GELU()
    elif act_name == "gelutanh":
