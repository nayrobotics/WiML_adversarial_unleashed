...
from xRL.utils.model_util import save_model

INTERACTION = 0
class Evaluator:
    def __init__(self, cfg, log_dir, rollout_callback=None, create_task_env_func=None):
        cfg = deepcopy(cfg)
        self.cfg = cfg
        ...
        try:
            ...
            pass
        p = mp.Process(target=rollout_callback, args=(cfg, log_dir, self.child, create_task_env_func), daemon=True)
        p.start()
    ...
    def check_if_should_stop(self, step=None):
        if self.cfg.max_step is not None:
            ...
            #logger.warning("Current training time left: {} from Max time {}".format(time.time() - self.start_time), self.cfg.max_time)
            return (time.time() - self.start_time) > self.cfg.max_time

    def get_training_time(self):
        return time.time() - self.start_time
    
def default_rollout(cfg, log_dir, child, create_task_env_func=None):
    ...
    if create_task_env_func is None:
        from xRL.utils.leggedgym_util import create_task_env as create_task_env_func
    ...
    with torch.inference_mode():
        while True:
            ...
            if ret_mean > ret_max:
                ret_max = ret_mean
            """
            save_model(path=f"{log_dir}/model.pth",
