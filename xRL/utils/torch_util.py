...
def memory_manegement(allocator_config="caching_allocator"):
...
class RunningMeanStd:
    ...
    def normalize(self, x):
        out_ = (x - self.mean) 
        out = out_ / torch.sqrt(self.var + self.epsilon)
        return out
...
class Normalizer(RunningMeanStd):
    ...
    def normalize(self, input):
        return torch.clamp(
            ...
            min=-self.clip_obs, max=self.clip_obs)

    def normalize_torch(self, input, device):
        #mean_torch = torch.tensor(
        #    self.mean, device=device, dtype=torch.float32)
        mean_torch =  self.mean.clone().detach().to(dtype=torch.float32, device=device)
        std = self.var + self.epsilon
        #std_torch = torch.sqrt(torch.tensor(
        #    self.var + self.epsilon, device=device, dtype=torch.float32))
        std_torch = std.clone().detach().to(dtype=torch.float32, device=device) 
        return torch.clamp(
