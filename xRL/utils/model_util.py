...
def load_model(model, model_type, cfg):
    ...
    artifact.download(xRL.LIB_PATH)
    logger.warning(f'Load {model_type}')
    weights = torch.load(Path(xRL.LIB_PATH, "model.pth"))
    ...
    """
    logger.warning(f'Load {model_type}')
    checkpoint_dir = cfg.artifact
    print("checkpoint_dir", checkpoint_dir)
    # TODO: load the last model
    path =  os.path.join(os.getcwd(), checkpoint_dir, "model.pth") 
...
def save_model(path, actor, critic, rms, ret_max=None):
