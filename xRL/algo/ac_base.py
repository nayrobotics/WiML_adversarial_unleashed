...
class ActorCriticBase:
    ...
    def __post_init__(self):
        print("Initializing actor critic class...")
        ...
        self.actor = act_class(self.obs_dim, self.action_dim, self.cfg.algo.act_hidden_layers, self.cfg.algo.act_activation).to(self.cfg.device)
        self.critic = cri_class(self.obs_dim, self.action_dim, self.cfg.algo.cri_hidden_layers, self.cfg.algo.cri_activation).to(self.cfg.device)
        ...
        # Discriminator tracker
        self.disc_return_tracker = Tracker(self.cfg.algo.tracker_len)
        self.current_disc_returns = torch.zeros(self.cfg.num_envs, dtype=torch.float32, device=self.cfg.device)
    ...
    def update_additional_tracker(self, disc_reward, done):
        self.current_disc_returns += disc_reward
        env_done_indices = torch.where(done)[0]

        #discriminator tracker 
        self.disc_return_tracker.update(self.current_disc_returns[env_done_indices])

        self.current_disc_returns[env_done_indices] = 0
    ...
    def optimizer_update_mix_precision(self, optimizer, objective):
        optimizer.zero_grad(set_to_none=True)
