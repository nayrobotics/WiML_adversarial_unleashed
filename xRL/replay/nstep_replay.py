...
from xRL.replay.replay_buffer import create_buffer
...
class NStepReplay:
    def __init__(self, obs_dim: int,
        ...
        ret = create_buffer(capacity=(self.num_envs, self.nstep),
                            obs_dim=obs_dim, action_dim=action_dim, amp_dim=amp_dim,
                            device=device)
        self.nstep_buf_obs, self.nstep_buf_action, self.nstep_buf_next_obs, \
        self.nstep_buf_reward, self.nstep_buf_done, self.nstep_buf_amp_obs, self.nstep_buf_next_amp_obs = ret
        self.nstep_count = 0
        self.gamma = gamma
        self.gamma_array = torch.tensor([self.gamma ** i for i in range(self.nstep)]).to(device).view(-1, 1)

    @torch.no_grad()
    def add_to_buffer(self, obs, actions, rewards, next_obs, dones, amp_obs, next_amp_obs):
        if self.nstep > 1:
            obs_list, action_list, reward_list, next_obs_list, done_list, amp_obs_list, next_amp_obs_list = list(), list(), list(), list(), list(), list(), list()
            for i in range(obs.shape[1]):
                ...
                self.nstep_buf_amp_obs = self.fifo_shift(self.nstep_buf_amp_obs, amp_obs[:, i])
                self.nstep_buf_next_amp_obs = self.fifo_shift(self.nstep_buf_next_amp_obs, next_amp_obs[:, i])
                ...
                amp_obs_list.append(self.nstep_buf_amp_obs[:, 0])
                next_amp_obs_list.append(self.nstep_buf_next_amp_obs[:, 0])

            return torch.cat(obs_list), torch.cat(action_list), torch.cat(reward_list), torch.cat(next_obs_list), torch.cat(done_list), torch.cat(amp_obs_list), torch.cat(next_amp_obs_list)
        else:
            return obs, actions, rewards, next_obs, dones, amp_obs, next_amp_obs
